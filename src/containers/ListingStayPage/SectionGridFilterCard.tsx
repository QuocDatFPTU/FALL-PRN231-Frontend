...
const SectionGridFilterCard: FC<SectionGridFilterCardProps> = ({ className = '' }) => {
  ...
  useEffect(() => {
    (async () => {
      const destination = await (await destinationApi.getById(Number(id))).data.data;
      const tour = await (await tourApi.getAll({ destinationId: id })).data.data;
      setCard(
        tour
          .map((e : any) => {
            return {
              ...
              title: e.tourName,
              galleryImgs: e.destinationImages.map((b: any) => b.image).slice(0, 3),
              commentCount: 70,
              viewCount: 602,
              like: false,
              address: `${e.address.area.name}, ${destination.address.city.name}`,
              reviewStart: 4.8,
              reviewCount: 28,
